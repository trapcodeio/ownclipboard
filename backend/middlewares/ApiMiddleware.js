...
module.exports = {
    ...
    async allow(http) {
        ...
        const apiKeyNotFound = {
            ...
            message: `ApiKey not found in request.`
        };
        const apiKeyNotValid = {
            type: "api_key_not_valid",
            message: `ApiKey found but not valid.`
        };
        ...
        if (!api_key) return error(apiKeyNotFound, 401);
        ...
        const clipNotFound = {
            type: "clip_not_valid",
            message: `Clip not valid, maybe already deleted.`
        };
