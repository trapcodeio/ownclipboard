...
const AuthController = $.handler({
    ...
    // Boot AuthController
    boot: async (http) => {
        let username = http.$query.removeNullOrUndefined().get("username");

        if (!username) {
            username = http.$body.removeNullOrUndefined().get("username", username);
        }

        if (username) username = username.toLowerCase();

        const user =
            typeof username === "string" && !(username.length < DataSchema.username)
                ? await User.query().where({ username }).first()
                : false;
    ...
    checkUsername: async (http, boot, error) => {
        ...
        if (username && username.length < DataSchema.username) return error(`Username too short.`);
    ...
    login: async (http, boot, error) => {
        ...
        let password = http.$body.removeNullOrUndefined().get("password", "");
    ...
    change_password: (http, boot, error) => {
        ...
        if (!body.removeNullOrUndefined().exists(needed)) {
